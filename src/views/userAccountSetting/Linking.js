...
const Linking = (props) => {
  ...
  const [xboxAccount, setXboxAccount] = useState({
    isLinked: false,
    infoRegardingAccount: {}
  });
  const [playStationAccount, setPlayStationAccount] = useState({
    isLinked: false,
    infoRegardingAccount: {}
  });
  ...
  const handleAccountLink = async (e, consoleName) => {
    ...
    try {
      ...
      console.log(
        '🚀 ~ file: Linking.js ~ line 147 ~ handleAccountLink ~ error',
        error
      );
  ...
  const handleUnlinkAccount = async (consoleName) => {
    ...
    try {
      ...
      console.log(
        '🚀 ~ file: Linking.js ~ line 171 ~ handleUnlinkAccount ~ error',
        error
      );
      Sentry.captureException(error, {
        tags: {
  ...
  const getLinkedAccounts = async () => {
    try {
      const response = await getLinkedNetworkService({
      ...
      console.log(
        '🚀 ~ file: Linking.js ~ line 237 ~ getLinkedAccounts ~ error',
        error
      );
      Sentry.captureException(error, {
        tags: {
  ...
  const ModalWindow = () => {
    return (
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to unlink? The Events and challenges in which
            you've registered with this linked account would fail.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button
                ...
                <Box display="flex">
                  <FormControl>
                    <TextField
                      ...
                      variant="outlined"
                    />
                    <FormHelperText error>{errors.psnName}</FormHelperText>
                  </FormControl>
  ...
  return (
    <div>
      ...
      <Table>
        ...
        <TableBody>
          {/* xbox */}
          <TableRow spacing={0} hover key={'xbox'}>
            <TableCell
            ...
            <TableCell>
              {xboxAccount.isLinked
                ? `${xboxAccount.infoRegardingAccount.userNameOnNetwork}`
                : 'NA'}
            </TableCell>
          ...
          {/* battlenet */}
          <TableRow spacing={0} hover key={'battlenet'}>
            <TableCell
              className={classes.imageCell}
              align="center"
              padding="none"
            ...
            <TableCell>
              {battleAccount.isLinked
                ? `${battleAccount.infoRegardingAccount.userNameOnNetwork}`
                : 'NA'}
            </TableCell>
            <TableCell align="center">
              <Button
          ...
          {/* psn */}
          <TableRow spacing={0} hover key={'psn'}>
            <TableCell
              className={classes.imageCell}
              align="center"
              padding="none"
            ...
            <TableCell>
              {playStationAccount.isLinked
                ? `${playStationAccount.infoRegardingAccount.userNameOnNetwork}`
                : 'NA'}
            </TableCell>
            <TableCell align="center">
              <Button
        ...
        <AccordionDetails>
          <Box>
            ...
            <Typography variant="body2" display="block" gutterBottom>
              ...
              accounts on ChainGames Battle. Players using PSN or Activision
              accounts in COD: MW do not need to worry. Just follow these simple
              steps to link your account on ChainGames and start battling!
            </Typography>
            <Typography variant="body2" display="block" gutterBottom>
            ...
            <Typography variant="body2" display="block" gutterBottom>
              ...
              website with your existing PSN or Activision account (whichever
              one you use to play the game).
            </Typography>
            <Typography variant="body2" display="block" gutterBottom>
              At the top of the website, you'll see your username. Hover over it
              and select "Linked Accounts" from the pop-up menu.
            </Typography>
            <Typography variant="body2" display="block" gutterBottom>
              You will now see options to link with other gaming networks.
              Select "Battle.net" from that list and login to your Battle.net
              account. This will link your exisitng PSN/Activision account with
              Battle.net and sync your COD: MW stats accross the two.
            </Typography>
            <Typography variant="body2" display="block" gutterBottom>
              Now you can link your Battle.net account here. That's it. Our
              systems will now be able to see your stats even when you're
              playing on your PlayStation or using your Activision account.
              Battle on!
            </Typography>
          </Box>
        </AccordionDetails>
