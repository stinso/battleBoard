let networkName = "",
  ...
  gameNetworksWithNameAndID = [
    { name: "Xbox Live", index: 0, devicesSupported: [devices.XBOX_ONE, devices.XBOX_SERIES] },
    { name: "BattleNet", index: 1 },
    { name: "PlayStation (PSN)", index: 2, devicesSupported: [devices.PS4, devices.PS5] },
    { name: "Activision", index: 3 },
  ],
  allSupportedGames = [
    {
      name: "Call of Duty: Modern Warfare", id: 1,
      shortName: 'COD: MW',
      supportedNetworks: [gameNetworksWithNameAndID[0], gameNetworksWithNameAndID[1]]
    },
    {
      name: "Madden NFL 21", id: 2,
...
export const GameFormat = {
  ...
  [allSupportedGames[1].name] : [
    { name: "Max Score", index: 0 },
  ],
  [allSupportedGames[2].name] : [
    { name: "Max Score", index: 0 },
  ],
};
export const passwordRegex = /^(?=.*\d)(?=.*[!@#$%^&*()_+=,.\\\/;':"-])(?=.*[a-z])(?=.*[A-Z]).{6,30}$/;
export const BetAmount = [1, 2.5, 5, 10, 15, 20, 25];
export const MaxWinners = [1, 2, 3, 5, 10];
export const MAX_APPROVED_BALANCE = 1.157920892373162e+59;
