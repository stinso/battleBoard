...
const Prerequisites = ({
  ...
  useEffect(()=> {
    if(rewardAmount) {
      dispatch({ type: SET_STAKING_VALUE, payload: ethers.utils.formatEther(rewardAmount.toString())});
    }
  }, [rewardAmount]);
    ...
    return (
      <Formik
        initialValues={{
          ...
          rewardsAccumulated: (rewardAmount != 0 && stakedDate.toString() && acc.loggedIn) ? true : false,
        ...
        validationSchema={Yup.object().shape({
          ...
          rewardsAccumulated: Yup.boolean().oneOf([true], "Must have CHAIN rewards"),
      ...
      {({
        ...
        >
          {withdrawDate !== '0' && !sevenDaysPassed && isAlertVisible && acc.loggedIn && (
            <Box mb={3}>
              <Alert
                ...
                severity="info"
              >
                You will be able to stake again after proceeding with the final withdrawal step on the <strong>{moment.utc(moment.unix(withdrawDate).add(7, 'days')).local().format("Do of MMMM HH:mm A")} {timezone}</strong>,
          ...
          <Box mt={2}>
            ...
            >
              <Radio
                checked={values.rewardsAccumulated}
              />
              <Box ml={2}>
                <Typography
                  gutterBottom
                  ...
                  color="textPrimary"
                >
                  Reward Accumulated {ethers.utils.formatEther(rewardAmount.toString())} CHAIN
                </Typography>
                <Typography
                  variant="body1"
                  color="textPrimary"
                >
                  Your CHAIN staking rewards
                </Typography>
              </Box>
            </Paper>
            <Paper
